...
export default function Home(){
 ...
 return <main className="site-shell">
  ...
  <section className="catalog-layout" id="viewer">
   <aside className="character-list">{filtered.map(c=><button key={c.code} className={`character-card ${selected.code===c.code?"selected":""}`} onClick={()=>setSelected(c)} style={{"--accent":c.color} as React.CSSProperties}><span className="card-image"><img src={`/characters/thumbs/${c.image}`} alt=""/></span><span className="card-info"><small>{c.code} // {c.group.toUpperCase()}</small><strong>{c.name}</strong><span>{c.role}</span></span><i/></button>)}</aside>
   <article className="viewer-panel" style={{"--accent":selected.color} as React.CSSProperties}><div className="viewer-header"><div><span>MODEL// RIG_TEST_CORE.GLB</span><h2>{selected.name}</h2></div><span className="stage-badge">3D RIG // ONLINE</span></div><div className="model-stage"><div className="stage-grid"/><span className="corner tl"/><span className="corner tr"/><span className="corner bl"/><span className="corner br"/><ThreeViewer animation={animation} accent={selected.color}/><div className="model-overlay"><span>REALTIME // WEBGL</span><span>RIG: TEST_HUMANOID</span></div></div><div className="animation-bar" id="mocap"><span>ANIMAÇÃO</span><div>{animations.map(item=><button key={item} className={animation===item?"active":""} onClick={()=>setAnimation(item)}>{item}</button>)}</div><span className="animation-state">PREVIEW: {animation}</span></div></article>

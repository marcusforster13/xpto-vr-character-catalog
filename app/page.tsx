...
export default function Home(){
 ...
 const isFederal=selected.code==="PF-03";
 const modelUrl=isFederal?"/models/PF-03-policia-federal.glb":"https://threejs.org/examples/models/gltf/Soldier.glb";
 const federalAnimations:Record<string,string>={IDLE:"/animations/PF-03/Idle.fbx",WALK:"/animations/PF-03/Walking.fbx",RUN:"/animations/PF-03/Running.fbx"};
 const animationUrl=isFederal?(federalAnimations[animation]??federalAnimations.IDLE):undefined;
 return <main className="site-shell">
  ...
  <section className="catalog-layout" id="viewer">
   <aside className="character-list">{filtered.map(c=><button key={c.code} className={`character-card ${selected.code===c.code?"selected":""}`} onClick={()=>{setSelected(c);setAnimation("IDLE")}} style={{"--accent":c.color} as React.CSSProperties}><span className="card-image"><img src={`/characters/${c.image}`} alt=""/></span><span className="card-info"><small>{c.code} // {c.group.toUpperCase()}</small><strong>{c.name}</strong><span>{c.role}</span></span><i/></button>)}</aside>
   <article className="viewer-panel" style={{"--accent":selected.color} as React.CSSProperties}><div className="viewer-header"><div><span>MODEL// {isFederal?"PF-03_POLICIA_FEDERAL.GLB":"RIG_TEST_CORE.GLB"}</span><h2>{selected.name}</h2></div><span className="stage-badge">3D RIG // ONLINE</span></div><div className="model-stage"><div className="stage-grid"/><span className="corner tl"/><span className="corner tr"/><span className="corner bl"/><span className="corner br"/><ThreeViewer animation={animation} accent={selected.color} modelUrl={modelUrl} modelLabel={isFederal?"PF-03 // MODELO XPTO":"RIG HUMANOIDE DEMO // THREE.JS SOLDIER"} animationUrl={animationUrl}/><div className="model-overlay"><span>REALTIME // WEBGL</span><span>RIG: {isFederal?"PF-03 MIXAMO":"TEST_HUMANOID"}</span></div></div><div className="animation-bar" id="mocap"><span>ANIMAÇÃO</span><div>{animations.map(item=><button key={item} className={animation===item?"active":""} disabled={isFederal&&!federalAnimations[item]} onClick={()=>setAnimation(item)}>{item}</button>)}</div><span className="animation-state">PREVIEW: {animation}</span></div></article>
